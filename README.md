...
GEPMC-Loc is a dynamic gated ensemble framework for RNA subcellular localization prediction, fusing pre-trained language models with attention-enhanced multi-scale convolutional neural networks. It combines structure-aware semantic representations from ERNIE-RNA, cross-modal evolutionary features from ProtRNA, and local sequence patterns, and employs a dynamic gating mechanism for adaptive multi-source feature fusion to improve the modeling of diverse RNA localization signals.
...
**Important**: You must follow the instructions provided in the links above to configure the required environments for both ERNIE-RNA and ProtRNA to enable subsequent feature extraction.
...
### Step 5: Inference and Evaluation (Direct Prediction)
If you want to directly predict, you can use our saved models for prediction. After downloading and extracting the feature files, your directory structure should look like this:

```plaintext
GEPMC-Loc/
├── data/                       # Dataset files (.pkl)
├── ERNIE_RNA_embedding/        # Extracted ERNIE-RNA features
├── ProTRNA_embedding/          # Extracted ProtRNA features
├── save_model/                 # Saved models for prediction
