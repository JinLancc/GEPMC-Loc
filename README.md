...
### Step 1: Clone Pre-trained Repositories
...
To evaluate the model on the independent test set using the ensemble strategy:
```bash
python predict_GEPMC_Loc.py
```
**Prediction Strategy**: This script implements a **Cross-Validation Averaging** strategy. It loads the best models saved from all 5 folds, performs inference on the test set with each, and calculates the **average of the 5-fold results** to produce the final evaluation metrics (ACC, MCC, Sn, Sp, etc.). This approach ensures a more stable and generalized assessment of the model's performance.
