...
def seq_to_index(sequences):
    ...
    sequences = [s[:1022] for s in sequences]
...
def extract_embedding_of_ernierna(sequences, if_cls=True,
                                  arg_overrides={"data": '/data/code/BERT/onestage_checkpoint_dict/'},
                                  pretrained_model_path='/data/code/BERT/Pretrain_checkpoint/twocheckpoint_best.pt',
                                  device='cpu', layer_idx=12):
    '''
    input:
    sequences: List of string (difference length)
    ...
    if if_cls:
        ...
        embedding = np.zeros((len(sequences), layer_num, 1024, 768))
...
def extract_attnmap_of_ernierna(sequences, attn_len=None,
                                arg_overrides={"data": '/data/code/BERT/onestage_checkpoint_dict/'},
                                pretrained_model_path='/data/code/BERT/Pretrain_checkpoint/twocheckpoint_best.pt',
                                device='cpu', layer_idx=13, head_idx=12):
    '''
    input:
    sequences: List of string (difference length)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pkl_path", required=True, type=str, help="Input PKL path")
    parser.add_argument("--save_path", required=True, type=str, help="Output NPY file path")
    parser.add_argument("--arg_overrides", default={"data": './src/dict/'}, help="Vocabulary path")
    parser.add_argument("--ernie_rna_pretrained_checkpoint",
                        default='./checkpoint/ERNIE-RNA_checkpoint/ERNIE-RNA_pretrain.pt', type=str)

    parser.add_argument("--layer_idx_emb", default=11, type=int)
    ...
    print(f"Loading sequences from {args.pkl_path}...")
    with open(args.pkl_path, "rb") as f:
        seqs_lst = pickle.load(f)

    device_str = f"cuda:{args.device}" if torch.cuda.is_available() else 'cpu'
    all_embedding = extract_embedding_of_ernierna(
        seqs_lst,
        if_cls=True,
        arg_overrides=args.arg_overrides,
        pretrained_model_path=args.ernie_rna_pretrained_checkpoint,
        device=device_str,
        layer_idx=args.layer_idx_emb
    )

    original_shape = all_embedding.shape
    if all_embedding.ndim == 3 and all_embedding.shape[1] == 1:
        all_embedding = np.squeeze(all_embedding, axis=1)
        print(f"Squeezed feature dimension from {original_shape} to {all_embedding.shape}")

    np.save(args.save_path, all_embedding)
    print(f'Done in {time.time() - start:.2f}s! Saved to {args.save_path}')